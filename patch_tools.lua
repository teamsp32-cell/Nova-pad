...
-- Features: Clipboard, Find/Replace, Curtain, Smart Tools (14 LANGUAGES GLOBAL EDITION)
...
local ok, err = pcall(function()
    ...
    -- =======================================================
    -- 🌍 1. THE ULTIMATE UNIVERSAL DICTIONARY (14 Languages)
    -- =======================================================
    local langData = {
    ...
    local function getPatchLang()
        ...
        if ok_file and f then 
            ...
            if content then 
                local code = content:match("%a%a") -- Get first two letters (e.g., 'hi', 'en', 'id')
    ...
    -- =======================================================
    -- 🌟 2. हमारे नए जादुई फीचर्स (NEW FEATURES)
    -- =======================================================
        ...
        pcall(function()
            ...
            lv.setOnItemClickListener(AdapterView.OnItemClickListener{
                onItemClick = function(p,v,pos,id)
                    ...
                    -- 🔥 Smart Check: अगर क्लिपबोर्ड में कोई भी 'empty' शब्द है, तो एरर दिखाओ
                    local isSlotEmpty = false
                    for _, lData in pairs(langData) do
                        if _G.betaClipboard[pos+1] == lData.empty then isSlotEmpty = true; break end
                    end
    ...
    _G.showFindReplace = function()
        pcall(function()
            ...
            AlertDialog.Builder(publicActivity).setTitle(L("fnr_title")).setView(l).setPositiveButton(L("replace_all"), function()
                ...
                if not btnOk then AlertDialog.Builder(publicActivity).setTitle("🚨 Error").setMessage(tostring(btnErr)).setPositiveButton("OK", nil).show() end
            end).show()
        end)
    end
    ...
    -- =======================================================
    -- 🎯 3. THE MASTER OVERRIDE (AUTO-TRANSLATING MENU)
    -- =======================================================
    ...
    _G.openSmartTextCleaner = function()
        ...
        -- 🌟 यह लाइन यूज़र की भाषा के हिसाब से मेनू उठाएगी! 🌟
        local opts = langData[_G.appLanguage].tools
        local menuTitle = langData[_G.appLanguage].menu_title
        ...
        showNovaMenu(menuTitle, opts, function(w)
            ...
            if w == 0 then
                if #text == 0 then Toast.makeText(activity, "Error", 0).show(); return end
            ...
            elseif w == 1 then
                ...
                if #links > 0 then activity.getSystemService(Context.CLIPBOARD_SERVICE).setPrimaryClip(ClipData.newPlainText("Links", table.concat(links, "\n"))); Toast.makeText(activity, #links.." Copied!", 0).show() else Toast.makeText(activity, "Not found.", 0).show() end
            elseif w == 2 then noteEditor.setText(jText.replaceAll("[*#_~`|^]", "")); Toast.makeText(activity, "Success!", 0).show()
            elseif w == 3 then noteEditor.setText(jText.replaceAll("[\\x{1F300}-\\x{1F6FF}|\\x{2600}-\\x{26FF}|\\x{2700}-\\x{27BF}|\\x{1F900}-\\x{1F9FF}|\\x{1F1E6}-\\x{1F1FF}]", "")); Toast.makeText(activity, "Success!", 0).show()
            elseif w == 4 then local ft = jText.replaceAll(" +", " "); ft = ft.replaceAll("([.,])([A-Za-z\\u0900-\\u097F])", "$1 $2"); noteEditor.setText(ft.trim()); Toast.makeText(activity, "Success!", 0).show() 
            elseif w == 5 then 
                if #text == 0 then Toast.makeText(activity, "Error", 0).show(); return end
                ...
                showNovaMenu("TTS Options", ttsOpts, function(tIdx)
                    ...
                    elseif tIdx == 3 then if _G.tts_player then _G.tts_player.stop() end; Toast.makeText(activity, "Stopped ⏹️", 0).show()
                    else
                        Toast.makeText(activity, "Starting... 🗣️", 0).show()
            ...
            elseif w == 6 then noteEditor.setText(string.upper(text)); Toast.makeText(activity, "UPPERCASE! 🔠", 0).show()
            elseif w == 7 then noteEditor.setText(string.lower(text)); Toast.makeText(activity, "lowercase! 🔡", 0).show()

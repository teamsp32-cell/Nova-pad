...
-- Features: Clipboard, Find/Replace (Regex Hindi Fix), Curtain + Original Smart Tools
...
local ok, err = pcall(function()
    ...
    -- 🚨 JAVA REGEX FIND & REPLACE (100% Hindi/Space Fix) 🚨
    _G.showFindReplace = function()
        pcall(function()
            ...
            AlertDialog.Builder(publicActivity).setTitle(L("fnr_title")).setView(l).setPositiveButton(L("replace_all"), function()
                ...
                if not btnOk then 
                    AlertDialog.Builder(publicActivity).setTitle("🚨 Replace Error").setMessage(tostring(btnErr)).setPositiveButton("OK", nil).show() 
                end
